...
async function scrapeGitHubTopics(number) {
    ...
    try {
        ...
        while (topicNumber < number) {
            ...
            const allTopicsData = await topics.evaluateAll((topics) => topics.map((topic) => {
            ...
            for (let i = 0; i < allTopicsData.length && topicNumber < number; i++) {
                ...
                const repoData = await repositories.evaluateAll((repos, topic) => repos.map((repo) => {
                    ...
                    const tags = Array.from(repo.querySelectorAll('a.topic-tag')).map((tag) => tag.textContent.trim() // Get tags associated with the repository
                    );
